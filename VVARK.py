...
import numpy as np
from st_aggrid import AgGrid, GridOptionsBuilder  # در صورت نیاز؛ اگر نیاز ندارید می‌توانید حذف کنید.
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        for pos, (q_text, opts) in enumerate(default_questions, start=1):
            # ثبت سوالات پیش‌فرض به صورت "single"
...
# تابع load_questions شماره (position) هر سوال را هم دریافت می‌کند.
def load_questions():
    conn = sqlite3.connect('questions.db', timeout=10)
    c = conn.cursor()
...
def add_question(q_text, opts, position, q_type):
    ...
    try:
        ...
        # استفاده از offset موقت جهت جلوگیری از خطای UNIQUE
...
if user_role == "سوپروایزر":
    ...
    if st.session_state.get("admin_logged_in", False):
        ...
        st.subheader("🗑️ پاک کردن تمام داده‌های ثبت‌شده")
        file_name = "vark_results.xlsx"
        ...
        with st.expander("افزودن سوال جدید"):
            with st.form("add_question_form"):
                q_text = st.text_area("متن سوال:")  # لطفاً از نوشتن عبارت «سوال ...» استفاده نکنید.
                new_options = []
                if q_type_selected == "توضیحی":
                    st.info("برای سوال توضیحی نیازی به وارد کردن گزینه نیست.")
                elif q_type_selected == "4 گزینه‌ای":
                    st.write("لطفاً گزینه‌های موردنظر را وارد کنید (هر تعداد که پر کنید، همان تعداد نمایش داده می‌شود):")
                    ...
                    count = int(st.session_state.get("num_options_multiple", 2))
                ...
                if submitted:
                    if not q_text:
                        ...
                        # تعیین نوع نهایی: برای "توضیحی" همان؛ برای "4 گزینه‌ای" از نوع single (چون فقط یک جواب ثبت شود)
                        # و برای "چند گزینه‌ای" از نوع multiple
                        if q_type_selected == "توضیحی":
                            final_type = "توضیحی"
                        elif q_type_selected == "4 گزینه‌ای":
                            final_type = "single"
    ...
    if "user_info_submitted" not in st.session_state:
        ...
        if st.session_state.get("finished", False):
            ...
            if st.session_state.current_index < len(questions):
                ...
                if q_type == "توضیحی":
                    answer = st.text_area("Your Answer:", key=f"q{st.session_state.current_index}")
                    if st.button("Submit Answer"):
                        submit_answer(answer)
                elif q_type == "single":
                    # در سوالات single (4 گزینه‌ای) از st.radio استفاده می‌شود تا تنها یک گزینه ثبت شود.
                    answer = st.radio("Choose your Answer", options, key=f"q{st.session_state.current_index}",
                                      on_change=lambda: submit_answer(st.session_state.get(f"q{st.session_state.current_index}")))
                elif q_type == "multiple":
                    answer = st.multiselect("Choose your Answer (you can select multiple options)", options,
                                            key=f"q{st.session_state.current_index}")
                    if st.button("Submit Answer"):
                        submit_answer(answer)
                else:
                    answer = st.multiselect("Choose your Answer", options, key=f"q{st.session_state.current_index}")
                    if st.button("Submit Answer"):
                        submit_answer(answer)
            else:
                st.error("All questions have been answered or there is a numbering issue.")
