...
import numpy as np
from st_aggrid import AgGrid, GridOptionsBuilder  # در صورت نیاز، اگر استفاده نمی‌کنید می‌توانید حذف کنید.
...
# تابع load_questions: شماره (position) هر سوال را به همراه بقیه اطلاعات بازیابی می‌کند.
def load_questions():
    conn = sqlite3.connect('questions.db', timeout=10)
    c = conn.cursor()
...
# ---------------------------
init_db()
# برای سوالات تک‌گزینه‌ای از dummy_option استفاده نمی‌کنیم؛
# کافی است بالا یک پیام راهنما نمایش داده شود.
# dummy_option = "گزینه مورد نظر را انتخاب کنید"
        ...
        with st.expander("افزودن سوال جدید"):
            with st.form("add_question_form"):
                ...
                if q_type_selected == "توضیحی":
                    ...
                    st.write("لطفاً گزینه‌های موردنظر را وارد کنید (هر تعداد پر شود، همان تعداد نمایش داده می‌شود):")
        ...
        if st.session_state.get("finished", False):
            ...
            if st.session_state.current_index < len(questions):
                ...
                if q_type == "توضیحی":
                    ...
                    st.info("لطفاً یک گزینه را انتخاب کنید:")
